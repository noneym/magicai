...
function initChat() {
	...
	$('.chats-container').find('pre[class*=language-] code').each((i, el) => {
...
function highlightCode( wrapperEl ) {
	...
	let codeOutputText = wrapperEl.innerHTML;

	const codeBlocks = codeOutputText.match(/```[A-Za-z_]*\n[\s\S]+?```|```[A-Za-z_]*<br\s*\/?>[\s\S]+?```/g);

	if (!codeBlocks) return;

	codeBlocks.forEach((block) => {
		const language = block.match(/```([A-Za-z_]*(\n|<br\s*\/?>))/)[1].replace(/<br\s*\/?>/g, '');
		const code = block
			.replace(/```[A-Za-z_]*(\n|<br\s*\/?>)/, '')
			.replace(/```/, '')
			.replace(/<br\s*\/?>/g, '\n')
			...
			.replace(/"/g, '&quot;')
			.replace(/'/g, '&#039;');
		const wrappedCode = `<pre class="line-numbers rounded [direction:ltr]"><code class="language-${language}">${code}</code></pre>`;
		codeOutputText = codeOutputText.replace(block, wrappedCode);
	});

	wrapperEl.innerHTML = codeOutputText;
