@foreach ($chat->messages as $message)
    ...
    @if ($message->input != null)
        <div class="lqd-chat-user-bubble mb-2.5 flex flex-row-reverse content-end gap-2 last:mb-0 lg:ms-auto">
            @php
    ...
    <div class="lqd-chat-ai-bubble mb-2.5 flex content-start gap-2 last:mb-0">
        <span
            class="size-6 inline-block shrink-0 rounded-full bg-cover bg-center"
        ...
        @if ($message->output != null)
            <div class="chat-content-container group relative max-w-[calc(100%-64px)] rounded-[2em] bg-clay text-heading-foreground dark:bg-white/[2%]">
                @php
                    ...
                    $output = str_replace(['<br><br>', '<br/><br/>', '<br ><br >', '<br /><br />', '<br>', '<br/>', '<br >', '<br />'], "\n", $output);
                    $output = preg_replace_callback(
                        '/```(.*?)```/s',
                        function ($matches) {
                            $codeBlock = $matches[1];
                            $language = '';
                            preg_match('/^([^\n]+)/', $codeBlock, $languageMatches);
                            if (count($languageMatches) > 1) {
                                $language = trim($languageMatches[1]);
                                $codeBlock = str_replace($languageMatches[0], '', $codeBlock);
                            }
                            return '<pre class="line-numbers language-' . $language . ' rounded [direction:ltr]"><code>' . trim($codeBlock) . '</code></pre>';
                        },
                        $output,
                    );
                @endphp
                <pre
                    class="chat-content relative w-full whitespace-pre-wrap px-6 py-3 indent-0 font-[inherit] text-xs font-normal [word-break:break-word] empty:hidden [&_p]:mb-4 [&_p]:last:mb-0 [&_pre]:!mt-4 [&_pre]:-mb-2">{!! $output !!}</pre>
                <button
